...
export async function getImportedReviewSummaries(start:string,end:string,scope?:string[]){
 ...
 const locations=selected.map(location=>{const row=rows.find(item=>String(item.location)===location);if(!row)return{location,reviewCount:0,averageRating:null,fiveStarCount:0,lowRatingCount:0,unansweredCount:null,minimumMet:false,scorePct:0};const reviewCount=Number(row.review_count)||0,ratingSum=Number(row.rating_sum)||0,averageRating=reviewCount?ratingSum/reviewCount:null,minimumMet=reviewCount>=5,quality=averageRating===null?0:Math.max(0,Math.min(1,averageRating-4)),volume=Math.min(1,reviewCount/10),scorePct=minimumMet?Math.min(100,(quality*.8+volume*.2)*100):0;return{location,reviewCount,averageRating,fiveStarCount:Number(row.five_star_count)||0,lowRatingCount:Number(row.low_rating_count)||0,unansweredCount:null,minimumMet,scorePct};});
 return{source:'Vista Social CSV',start,end,minimumReviews:5,scoring:{qualityWeight:80,volumeWeight:20,volumeTarget:10},locations,hasData:rows.length>0,lastUpdated:rows.length?new Date(Math.max(...rows.map(row=>new Date(String(row.updated_at)).getTime()))).toISOString():null};
}
