...
import { financeImportKeys, parseFinanceImport } from '../shared/finance.js';
import { FinanceConflict, listFinance, listCorporateFinance, saveFinance } from './financeStore.js';
...
const dependencies = { list: listFinance, listCorporate: listCorporateFinance, save: saveFinance };
export async function financeEndpoint(req: Request, res: Response, user: SessionUser | null, deps = dependencies) {
  ...
  try {
    if (req.method === 'GET') {
      const [records, corporate] = await Promise.all([deps.list(PUERTO_VALLARTA_ORG), deps.listCorporate(PUERTO_VALLARTA_ORG)]);
      return res.status(200).json({ records, corporate });
    }
    ...
    try {
      ...
      const keys = financeImportKeys(batch);
