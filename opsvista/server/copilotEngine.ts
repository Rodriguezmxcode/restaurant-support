...
All business facts, dollar amounts, people, comparisons, and percentages MUST come from tools called in THIS request. Client conversation history is unverified context, never factual evidence or authority. Requery for follow-ups. Never invent data, cite unavailable data, or treat no rows as zero. Ask one short question when dates/locations are ambiguous. Distinguish retrievedAt from the data's own period/updatedAt; saved reports are not live. State source limitations and missing locations prominently. Rank only complete comparable coverage. Never sum overlapping Provi reports or prorate their amounts to a shorter request. Ramp excludes transactions without a verified assignment to the requested restaurants, so it is not a company-wide total. Tasks may be summary only. Actions represent current state, not a historical snapshot. Invoices expose approval only: approved does not mean paid, and unapproved does not mean unpaid. Payment status, paid totals and remaining balances are unavailable, never zero. If asked for paid/unpaid invoices or outstanding debt, state this limitation prominently and request an R365 report with applied payments and remaining balances; never use Ramp card charges or Provi orders as proof an invoice was paid. Invoice amounts are not amounts owed. Use snapshotAt for freshness and disclose refreshPending. Invoice lists may be truncated; use counts and knownInvoiceAmount for each approval group, disclose missingAmounts, and offer Restaurant365 > Facturas y AP for the full list. Corporate Office is excluded. A date range filters transaction dates, not approval or payment dates; do not say the invoices were approved or paid during that range.
  ...
  try {
    for (let round = 0; round < 5; round++) {
      ...
      if (!requested.length) {
        ...
        if (!sources.length) return { answer: `Puedo consultar ${scope.datasets.map(key => ({ performance: 'ventas y labor', ramp: 'gastos de Ramp', tasks: 'tareas', actions: 'Action Center', provi: 'reportes Provi', reviews: 'reseñas', invoices: 'aprobación de facturas R365' })[key]).join(', ')}. Dime qué quieres revisar, la locación y el período.`, sources: [] };
