...
type ApiRequest={method?:string;headers?:Record<string,string|string[]|undefined>&{cookie?:string;authorization?:string};query?:Record<string,string|string[]>;body?:Record<string,unknown>};
...
async function projects(req:ApiRequest,res:ApiResponse,user:NonNullable<ReturnType<typeof readSession>>){
 ...
 if(req.method==='PUT'){
  ...
  const project=await updateProjectRecord(id,{...(status?{status}:{}),...(priority?{priority}:{}),...(has(req.body,'ownerName')?{ownerName:text(req.body?.ownerName)||undefined}:{}),...(has(req.body,'progress')?{progress:Math.max(0,Math.min(100,Number(req.body?.progress)||0))}:{}),...(has(req.body,'actualSpend')?{actualSpend:Math.max(0,Number(req.body?.actualSpend)||0)}:{}),...(Array.isArray(req.body?.milestones)?{milestones:req.body.milestones as ProjectMilestone[]}:{})},text(