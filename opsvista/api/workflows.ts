import { financeEndpoint } from '../server/financeEndpoint.js';
import { partnerApiEndpoint, partnerKeyEndpoint, pvControlBrowserEndpoint } from '../server/partnerApi.js';
import { copilotEndpoint } from '../server/copilotEndpoint.js';
...
import { dispatchActionPush, dispatchOperationalPush, escalateUnacceptedActions, getActionNotification, getNotificationPreferences, getNotificationEmailStatus, sendNotificationEmailTest, recordActionReceipt, registerMobileDevice, updateNotificationPreferences, type ActionReceiptStatus } from '../server/actionNotificationStore.js';
...
async function notificationTest(req:ApiRequest,res:ApiResponse,user:NonNullable<ReturnType<typeof readSession>>){
 if(req.method!=='POST'){res.setHeader?.('Allow','POST');return res.status(405).json({error:'Method not allowed'});}
 if(req.body?.channel==='email')return res.status(200).json(await sendNotificationEmailTest(user));
...
async function notificationEmailStatus(req:ApiRequest,res:ApiResponse,user:NonNullable<ReturnType<typeof readSession>>){
 if(req.method&&req.method!=='GET'){res.setHeader?.('Allow','GET');return res.status(405).json({error:'Method not allowed'});}
 return res.status(200).json(await getNotificationEmailStatus(user));
}
