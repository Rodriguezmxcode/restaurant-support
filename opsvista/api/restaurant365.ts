...
export default async function handler(req:ApiRequest,res:ApiResponse){
  ...
  res.setHeader?.('X-OpsVista-R365-Version','r365-api-v2');
  ...
  try{
    ...
    if(!req.method||req.method==='GET'){
      ...
      const month=query(req,'month')||'2026-08';
      if(!view)return res.status(200).json(await getRestaurant365Status(organizationId));
      if(view==='ledger')return res.status(200).json(await getRestaurant365Ledger(organizationId,month,query(req,'entity')||'Corporate Office'));
