...
export default async function handler(req:Req,res:Res){
  ...
  try{
    ...
    const totals=locations.reduce((acc,row)=>({
      ...
      hourlyHours:acc.hourlyHours+row.hourlyHours,overtimeHours:acc.overtimeHours+row.overtimeHours,hourlyLaborCost:acc.hourlyLaborCost+row.hourlyLaborCost,
    }),{netSales:0,discountAmount:0,voidAmount:0,hourlyHours:0,overtimeHours:0,hourlyLaborCost:0});
    const round=(n:number)=>Math.round((n+Number.EPSILON)*100)/100;
    return res.status(200).json({
      source:'Toast Standard API',start,end,locations,
      totals:{...Object.fromEntries(Object.entries(totals).map(([k,v])=>[k,round(v)])),discountPct:totals.netSales?round(totals.discountAmount/totals.netSales*100):0,voidPct:totals.netSales?round(totals.voidAmount/totals.netSales*100):0,laborPct:totals.netSales?round(totals.hourlyLaborCost/totals.netSales*100):0,splh:totals.hourlyHours?round(totals.netSales/totals.hourlyHours):null},
      notes:{salaryLabor:'Salary labor is not available from Toast Standard time entries when hourlyWage is null.',tasks:'Tasks require the 7shifts production feed.'}
    });
  }catch(error){
