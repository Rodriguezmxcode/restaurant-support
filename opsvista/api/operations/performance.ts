...
export default async function handler(req:Req,res:Res){
  ...
  try{
    ...
    const totals=locations.reduce((acc,row)=>({
      netSales:acc.netSales+row.netSales,discountAmount:acc.discountAmount+row.discountAmount,bonusDiscountAmount:acc.bonusDiscountAmount+row.bonusDiscountAmount,uberEatsDiscountAmount:acc.uberEatsDiscountAmount+row.uberEatsDiscountAmount,employeeDiscountAmount:acc.employeeDiscountAmount+row.employeeDiscountAmount,voidAmount:acc.voidAmount+row.voidAmount,
    ...
    }),{netSales:0,discountAmount:0,bonusDiscountAmount:0,uberEatsDiscountAmount:0,employeeDiscountAmount:0,voidAmount:0,hourlyHours:0,overtimeHours:0,regularLaborCost:0,overtimeLaborCost:0,hourlyLaborCost:0,salaryLaborCost:0,totalLaborCost:0});
    return res.status(200).json({
      ...
      notes:{
        ...
        bonusDiscounts:'Weekly Bonus excludes applied Toast discounts identified as Uber Eats or Employee/Staff by default. Total discounts remain unchanged in Sales and other modules.'
      }
    });
  }catch(error){
