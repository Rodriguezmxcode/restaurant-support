...
type Period='today'|'this-week'|'previous-week';
...
function selectedRange(key:Period){const now=new Date();const sinceWed=(now.getDay()-3+7)%7;const weekStart=new Date(now.getFullYear(),now.getMonth(),now.getDate()-sinceWed);if(key==='today')return{start:iso(now),end:iso(now)};if(key==='previous-week'){const start=new Date(weekStart);start.setDate(start.getDate()-7);const end=new Date(start);end.setDate(end.getDate()+6);return{start:iso(start),end:iso(end)}}return{start:iso(weekStart),end:iso(now)};}

export default function WeeklyBonusLivePanel({allowedLocations}:{allowedLocations?:string[]}){
  const [period,setPeriod]=useState<Period>(()=>{const saved=typeof window!=='undefined'?window.sessionStorage.getItem('opsvista-bonus-period'):null;return saved==='today'||saved==='previous-week'?saved:'this-week'});
  ...
  const range=useMemo(()=>selectedRange(period),[period]);
  ...
  useEffect(()=>{window.sessionStorage.setItem('opsvista-bonus-period',period)},[period]);
  ...
  useEffect(()=>{let cancelled=false;const run=async()=>{setLoading(true);const qs=new URLSearchParams({start:range.start,end:range.end});const [tr,sr]=await Promise.allSettled([fetch(`/api/tasks/weekly?${qs}`,{credentials:'include',cache:'no-store'}),fetch(`/api/operations/performance?${qs}`,{credentials:'include',cache:'no-store'})]);
    if(cancelled)return;
    ...
    setLoading(false);};void run();return()=>{cancelled=true};},[range.start,range.end]);
  ...
  return <section className="panel" style={{marginBottom:16}}><div className="panel-header"><div><h2>Bono semanal · Live Scorecard</h2><p>{range.start} → {range.end} · Todas las métricas corresponden al mismo periodo seleccionado.</p></div><div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}><span className="count-pill">{loading?'SYNCING':'LIVE SOURCES'}</span><select value={period} onChange={e=>setPeriod(e.target.value as Period)}><option value="today">Hoy</option><option value="this-week">Esta semana</option><option value="previous-week">Semana anterior</option></select><select value={selected} onChange={e=>setSelected(e.target.value)}><option>All locations</option>{scope.map(location=><option key={location}>{location}</option>)}</select></div></div><div style={{padding:18,display:'grid',gap:12}}>
    ...
    <div style={{fontSize:12,color:'#64748b'}}>El subtotal en vivo puede alcanzar 85 puntos: Tasks 35 + Discounts 20 + Voids 15 + OT 15. Liquor Cost (5) y Logbook/Leadership (10) permanecen separados hasta conectar sus fuentes de producción.</div>
  </div></section>;
}
