...
export default function ClientWorkspace({ user }: { user: OpsVistaUser }) {
  ...
  return <main className="client-workspace">
    ...
    <nav aria-label="Workspace"><button aria-pressed={tab === 'tasks'} onClick={() => setTab('tasks')}>Tasks & follow-up / Tareas</button>{user.role === 'Corporate' && <button aria-pressed={tab === 'team'} onClick={() => setTab('team')}>Team / Equipo</button>}<a href="tel:+12032850281">Support / Soporte</a></nav>
    {tab === 'team' ? <ClientOnboardingPanel currentUser={user}/> : <NativeTasksPanel currentUser={user} allowedLocations={locations} readOnly={false}/>}
  </main>;
}
