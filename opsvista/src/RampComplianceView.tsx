...
export default function RampComplianceView({ onEscalate }: Props) {
  const results = useMemo(() => evaluateRampCompliance(rampDemoTransactions), []);
  ...
  const escalate = (tx: RampComplianceResult) => {
    ...
    const severe = tx.complianceStatus === 'Critical';
    onEscalate?.({
      location,
  ...
  return <div className="ramp-page">
    <section className="ramp-summary-grid">
      <article className="ramp-summary-card hero-score">
        <span>RAMP COMPLIANCE SCORE</span>
        <strong>{summary.score}</strong><small>/100</small>
        <p>{summary.compliant} of {summary.total} transactions fully compliant</p>
      </article>
      ...
      <div className="ramp-table-wrap">
        <table className="ramp-table">
          ...
          <tbody>{filtered.map(tx => <tr key={tx.id} className={tx.complianceStatus === 'Critical' ? 'critical-row' : ''}>
            <td>{tx.date}</td>
            <td><strong>{tx.merchant}</strong><small>Ramp · {tx.state}</small></td>
            ...
            <td className="amount-cell">{money(tx.amount)}</td>
            <td><FlagPills transaction={tx}/></td>
    ...
    {selected && <div className="ramp-drawer-backdrop" onClick={() => setSelected(null)}>
      <aside className="ramp-drawer" onClick={e => e.stopPropagation()}>
        ...
        <div className="ramp-detail-grid">
          <div><label>Cardholder</label><strong>{selected.cardholder || 'Not identified'}</strong></div>
          <div><label>Department / Restaurant</label><strong>{selected.department || 'Unassigned'}</strong></div>
          <div><label>Memo</label><strong>{selected.memo || 'Missing'}</strong></div>
          <div><label>Receipt</label><strong>{selected.receiptAttached ? 'Attached' : 'Missing'}</strong></div>
        </div>
        ...
        <div className="ramp-drawer-actions">
          {selected.flags.length > 0 ? <button className="ramp-primary" disabled={escalated.includes(selected.id)} onClick={() => escalate(selected)}>{escalated.includes(selected.id) ? 'Added to Action Center' : 'Send to Action Center'}</button> : <button className="ramp-primary" disabled>Transaction compliant</button>}
