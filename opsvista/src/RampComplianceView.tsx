...
export default function RampComplianceView({ onEscalate }: Props) {
  ...
  const filtered = results.filter(tx => {
    ...
    const queryMatch = !q || [tx.merchant, tx.merchantLocation, tx.cardholder, tx.department, tx.restaurant, tx.entity, tx.category, tx.accountingCategory, tx.cardLastFour, tx.memo, ...tx.flags]
  ...
  return <div className="ramp-page">
    ...
    <section className="ramp-table-panel">
      ...
      <div className="ramp-table-wrap"><table className="ramp-table">
        <thead><tr><th>Date / time</th><th>Age</th><th>Cardholder / card</th><th>Merchant / place</th><th>Restaurant</th><th>Department</th><th>Category</th><th>Entity</th><th>Memo</th><th>Amount</th><th>Receipt / state</th><th>Compliance</th><th></th></tr></thead>
        ...
        </tr>)}{!filtered.length && <tr><td colSpan={13} className="ramp-empty-row">{transactions.length ? 'No transactions match the current search or status filter.' : 'Ramp returned no transactions for the selected period.'}</td></tr>}</tbody>
      </table></div>
    </section>
