...
export default function RampComplianceView({ onEscalate, allowedLocations, managerMode = false, initialQuery, initialRecordId }: Props) {
  ...
  const assigneesFor=(tx:RampComplianceResult)=>assignees.filter(user=>user.role==='Location Manager'&&user.locations.includes(tx.verifiedRestaurant||tx.restaurant||tx.department||''));
  const suggestedAssignee=(tx:RampComplianceResult)=>{const managers=assigneesFor(tx);return managers.length===1?managers[0]:undefined;};
