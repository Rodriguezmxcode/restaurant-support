...
export default function RampComplianceView({ onEscalate, allowedLocations, managerMode = false, initialQuery, initialRecordId }: Props) {
  ...
  const escalationFor = (tx: RampComplianceResult, assignee?:Assignee, dueAt?:string): Escalation => ({
    ...
    accountableName: tx.cardholder || 'Cardholder not identified',
    accountableRole: tx.role || 'Ramp cardholder',
    dueAt,
  ...
  const cardholderAssignee=(tx:RampComplianceResult)=>{const cardholder=normalizedPersonName(tx.cardholder||'');return cardholder?assignees.find(user=>normalizedPersonName(user.name)===cardholder):undefined;};
  const locationManagersFor=(tx:RampComplianceResult)=>assignees.filter(user=>user.role==='Location Manager'&&user.locations.includes(tx.verifiedRestaurant||tx.restaurant||tx.department||''));
  const assigneesFor=(tx:RampComplianceResult)=>{const cardholder=cardholderAssignee(tx);return [cardholder,...locationManagersFor(tx)].filter((user,index,users):user is Assignee=>Boolean(user)&&users.findIndex(candidate=>candidate?.id===user?.id)===index);};
  const suggestedAssignee=(tx:RampComplianceResult)=>cardholderAssignee(tx)||(()=>{const managers=locationManagersFor(tx);return managers.length===1?managers[0]:undefined;})();
  ...
  return <div className="ramp-page">
    ...
    {assignment&&<div className="ramp-assignment-backdrop" onMouseDown={()=>!assigning&&setAssignment(null)}><section className="ramp-assignment-dialog" role="dialog" aria-modal="true" aria-label="Assign Ramp compliance action" onMouseDown={event=>event.stopPropagation()}><header><div><span>ACCOUNTABILITY ASSIGNMENT</span><h2>Send to Action Center</h2><p>{assignment.transaction.cardholder||'Cardholder'} · {assignment.transaction.merchant} · {money(assignment.transaction.amount)}</p></div><button type="button" onClick={()=>setAssignment(null)} disabled={assigning}>×</button></header><div className="ramp-assignment-signal"><strong>{assignment.transaction.flags.map(flag=>rampFlagLabels[flag]).join(' · ')}</strong><span>{assignment.transaction.overdue?`Overdue by policy · ${assignment.transaction.ageHours}h old`:'Within the 48-hour compliance window'}</span></div><label>RESPONSIBLE PERSON<select value={assignment.ownerId} onChange={event=>setAssignment({...assignment,ownerId:event.target.value})}><option value="">Select responsible person</option>{assigneesFor(assignment.transaction).map(user=><option key={user.id} value={user.id}>{user.name} · {cardholderAssignee(assignment.transaction)?.id===user.id?'Cardholder':user.title}</option>)}</select></label>{cardholderAssignee(assignment.transaction)?<p className="ramp-assignment-match">✓ {cardholderAssignee(assignment.transaction)?.name} matches the Ramp cardholder and was selected directly.</p>:suggestedAssignee(assignment.transaction)&&<p className="ramp-assignment-match">✓ {suggestedAssignee(assignment.transaction)?.name} is the only active manager assigned to this location.</p>}<label>DUE DATE<OpsVistaDatePicker value={assignment.dueAt} onChange={dueAt=>setAssignment({...assignment,dueAt})} ariaLabel="Select compliance action due date"/></label><div className="ramp-assignment-result"><span>Action owner</span><strong>{assignees.find(user=>user.id===assignment.ownerId)?.name||'Select responsible person'} → My Actions</strong><small>{cardholderAssignee(assignment.transaction)?.id===assignment.ownerId?`Assigned directly to ${assignment.transaction.cardholder||'the cardholder'} for correction.`:`Follow-up with ${assignment.transaction.cardholder||'the cardholder'} to complete the correction.`}</small></div>{assignmentError&&<div className="ramp-assignment-error">{assignmentError}</div>}<footer><button type="button" onClick={()=>setAssignment(null)} disabled={assigning}>Cancel</button><button type="button" className="primary" onClick={()=>void sendAssignedAction()} disabled={assigning||!assignment.ownerId||!assignment.dueAt}>{assigning?'Assigning…':'Assign action'}</button></footer></section></div>}
  </div>;
}
