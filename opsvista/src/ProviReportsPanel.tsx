...
function ReportDetail({ report }: { report: StoredProviReport }) {
  ...
  return <details className="provi-report-detail"><summary>{report.location} · {usd(report.spend)} · {report.orders} órdenes · {report.distributorCount} distribuidores</summary>
    ...
    <details onToggle={event => setInvoicesOpen(event.currentTarget.open)}><summary>Ver {report.comparison.invoices.length} documentos de R365</summary>
      {invoicesOpen && <InvoiceTable invoices={report.comparison.invoices}/>}</details>
    {report.baseline && <details onToggle={event => setBaselineOpen(event.currentTarget.open)}><summary>Corte original de la revisión · {report.baseline.invoices.length} facturas · {stamp(report.baseline.capturedAt)}</summary>
      {baselineOpen && <><p>{report.baseline.note}</p><InvoiceTable invoices={report.baseline.invoices}/></>}</details>}
...
export default function ProviReportsPanel({ locations, allowImport = false }: { locations?: string[]; allowImport?: boolean }) {
  ...
  return <section className="provi-panel" aria-label="Compras Provi guardadas">
    <header><div><h3>Compras Provi · reportes guardados</h3><p>Referencia por locación y fechas, con revisión automática de facturas y créditos de Restaurant365.</p></div><button type="button" onClick={() => setReload(value => value + 1)} disabled={saving}>Consultar copia actualizada</button></header>
    ...
    {allowImport && data.canImport && <details className="provi-import"><summary>Cargar reportes Provi</summary>
      <p>Selecciona el archivo de reportes preparado para OpsVista. Revisa las fechas antes de guardar. La carga conserva los productos destacados y el corte de revisión de R365 cuando están disponibles.</p>
    ...
    {!loading && !data.reports.length && <p>Aún no hay reportes Provi guardados.</p>}
    {scoped.length > 0 && <><div className="provi-controls"><label>Período de Provi <select value={activePeriod} onChange={event => setPeriod(event.target.value)}>{periods.map(key => <option key={key}>{key}</option>)}</select></label><label>Locación de Provi <select value={activeLocation} onChange={event => setLocation(event.target.value)}><option value="">Todas las locaciones</option>{locationOptions.map(name => <option key={name}>{name}</option>)}</select></label></div>
      ...
      <p className="provi-note">Estos reportes conservan sus fechas originales, independientes del período del bono. Provi incluye bebidas sin alcohol y mezcladores. Sus precios pueden ser estimados; las diferencias requieren conciliación.</p>
      ...
      <p className="provi-warning">R365 se revisa aproximadamente cada 30 minutos y se concilia por la noche. Los reportes de Provi se actualizan mediante una nueva carga. Las diferencias y la compra neta todavía no representan utilidad ni asignan los 5 puntos del bono.</p>
