...
type InvoiceSort='oldest'|'newest'|'highest';
...
export default function Restaurant365View({canManageIntegrations}:{canManageIntegrations:boolean}){
  ...
  const visibleInvoices=useMemo(()=>{
    const rows=ap?.transactions.filter(row=>(invoiceEntity==='All entities'||row.entity===invoiceEntity)&&(invoiceStatus==='all'||(invoiceStatus==='approved'&&row.approved)||(invoiceStatus==='pending'&&!row.approved))&&(!normalizedSearch||`${row.number||''} ${row.name} ${row.vendor||''} ${row.entity||''} ${row.createdBy||''}`.toLowerCase().includes(normalizedSearch)))||[];
    return [...rows].sort((left,right)=>invoiceSort==='highest'?(right.amount||0)-(left.amount||0)||left.date.localeCompare(right.date):invoiceSort==='newest'?right.date.localeCompare(left.date)||right.createdOn?.localeCompare(left.createdOn||'')||0:left.date.localeCompare(right.date)||left.createdOn?.localeCompare(right.createdOn||'')||0);
  },[ap?.transactions,invoiceEntity,invoiceStatus,invoiceSort,normalizedSearch]);
  ...
  return <div className="r365-page">
    ...
    {tab==='Conexión'?<Restaurant365IntegrationPanel canManage={canManageIntegrations}/>:tab==='Resumen'?<>
      ...
      <div className="r365-metrics-grid"><Metric label="Facturas AP" value={String(ap.totals.invoices)} note={`${preciseMoney.format(ap.totals.amount)}${ap.totals.invoicesWithoutAmount?` · ${ap.totals.invoicesWithoutAmount} sin monto`:''}`}/><Metric label="Aprobadas" value={String(ap.totals.approved)} note={`${preciseMoney.format(ap.totals.approvedAmount)}${approvedWithoutAmount?` · ${approvedWithoutAmount} sin monto`:''}`} tone="good"/><Metric label="Pendientes" value={String(ap.totals.pending)} note={`${preciseMoney.format(ap.totals.pendingAmount)}${pendingWithoutAmount?` · ${pendingWithoutAmount} sin monto`:''}`} tone={ap.totals.pending?'warn':'good'}/><Metric label="Vendors" value={String(ap.totals.vendors)} note="Con factura en el periodo"/><Metric label="Locaciones" value={String(ap.totals.locations)} note="Restaurantes + Corporate"/></div>
      <section className="panel r365-card r365-ap-card">
        ...
        <div className="r365-ap-toolbar">
          <label><span>LOCACIÓN</span><select value={invoiceEntity} onChange={event=>{setInvoiceEntity(event.target.value);setSelectedInvoiceIds([]);setCopyNotice('');}}><option value="All entities">Todas las locaciones</option>{entities.map(item=><option key={item}>{item}</option>)}</select></label>
          ...
          <label><span>ORDEN DE PAGO</span><select value={invoiceSort} onChange={event=>setInvoiceSort(event.target.value as InvoiceSort)}><option value="oldest">Más viejos primero</option><option value="newest">Más recientes primero</option><option value="highest">Mayor monto primero</option></select></label>
          <label className="r365-ap-search"><span>BUSCAR INVOICE O VENDOR</span><input value={search} onChange={event=>{setSearch(event.target.value);setSelectedInvoiceIds([]);setCopyNotice('');}} placeholder="Número, vendor, locación o usuario…"/></label>
        ...
        <div className="r365-table-wrap"><table className="r365-ap-table"><thead><tr><th className="r365-check"><input type="checkbox" aria-label="Seleccionar todas las facturas visibles" checked={allVisibleSelected} onChange={toggleVisibleInvoices}/></th><th>Fecha invoice / factura</th><th>Antigüedad</th><th>Creada en R365</th><th>Vendor</th><th>Locación</th><th>Monto</th><th>Estado R365</th></tr></thead><tbody>{visibleInvoices.slice(0,500).map(row=><tr key={row.id} className={selectedInvoiceSet.has(row.id)?'selected':''}><td className="r365-check"><input type="checkbox" aria-label={`Seleccionar factura ${row.number||row.name}`} checked={selectedInvoiceSet.has(row.id)} onChange={()=>toggleInvoice(row.id)}/></td><td><strong>{dateLabel(row.date)}</strong><small>{row.number||row.name}</small></td><td><span className="r365-age">{invoiceAgeLabel(row.date)}</span></td><td><strong>{dateTimeLabel(row.createdOn)}</strong><small>{row.createdBy?`Cargada por ${row.createdBy}`:'Usuario no disponible'}</small></td><td>{row.vendor||'Sin vendor enlazado'}</td><td><strong>{row.entity||row.location}</strong><small>{row.location}</small></td><td className="amount">{row.amount===null?'No disponible':preciseMoney.format(row.amount)}</td><td><span className={`r365-status ${row.approved?'approved':'pending'}`}>{row.approved?'Aprobada':'Pendiente'}</span></td></tr>)}</tbody></table>{visibleInvoices.length>500&&<div className="r365-table-limit">Mostrando 500 de {visibleInvoices.length} facturas filtradas.</div>}{!visibleInvoices.length&&<div className="r365-empty">No se encontraron facturas con estos filtros.</div>}</div>
