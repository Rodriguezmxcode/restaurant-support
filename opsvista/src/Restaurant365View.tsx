...
export default function Restaurant365View({canManageIntegrations,priceWatchOnly=false}:{canManageIntegrations:boolean;priceWatchOnly?:boolean}){
  if(priceWatchOnly)return <div className="r365-page"><section className="panel r365-tabs" role="tablist" aria-label="Secciones de Restaurant365"/></div>;
  const [bonusRange]=useState(()=>{const start=window.sessionStorage.getItem('opsvista-r365-bonus-start')||'',end=window.sessionStorage.getItem('opsvista-r365-bonus-end')||'';return window.sessionStorage.getItem('opsvista-r365-open-alcohol')==='1'&&validBeverageRange(start,end,56)?{start,end}:undefined;});
  ...
  useEffect(()=>{
    if(['Resumen','Conexión','Compras Provi','Alcohol'].includes(tab)||(tab==='Corporate Office'&&!showCorporateLedger))return;
  ...
  },[tab,range.start,range.end,entity,reload,showCorporateLedger]);
