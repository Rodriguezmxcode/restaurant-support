...
type GaugeConfig={value:number;max:number;target:number;format:(value:number)=>string;higherIsBetter?:boolean};
function Gauge({config,status}:{config:GaugeConfig;status:'ready'|'pending'|'warning'}){
  const percent=Math.max(0,Math.min(100,config.value/config.max*100));
  const targetPercent=Math.max(0,Math.min(100,config.target/config.max*100));
  const angle=Math.PI*(1-targetPercent/100);
  const markerX=90+72*Math.cos(angle),markerY=90-72*Math.sin(angle);
  const meets=config.higherIsBetter?config.value>=config.target:config.value<=config.target;
  const tone=status==='pending'?'pending':meets?'good':'warning';
  return <div className={`overview-gauge ${tone}`} role="img" aria-label={`${config.format(config.value)}; target ${config.higherIsBetter?'at least':'at most'} ${config.format(config.target)}`}>
    <svg viewBox="0 0 180 108" aria-hidden="true">
      <path className="overview-gauge-track" d="M18 90 A72 72 0 0 1 162 90" pathLength="100" />
      <path className="overview-gauge-progress" d="M18 90 A72 72 0 0 1 162 90" pathLength="100" strokeDasharray="100" strokeDashoffset={100-percent} />
      <line className="overview-gauge-target" x1={markerX} y1={markerY-6} x2={markerX} y2={markerY+6} transform={`rotate(${targetPercent*1.8-90} ${markerX} ${markerY})`} />
    </svg>
    <strong>{config.format(config.value)}</strong><span>{meets?'Dentro del objetivo':'Requiere atención'}</span>
    <small>Meta {config.higherIsBetter?'≥':'≤'} {config.format(config.target)}</small>
  </div>;
}
function Kpi({label,value,note,status='ready',gauge,metric,active,disabled,onOpen}:{label:string;value:string;note:string;status?:'ready'|'pending'|'warning';gauge?:GaugeConfig;metric:OverviewMetric;active:boolean;disabled:boolean;onOpen:(metric:OverviewMetric)=>void}){
  const accent=status==='warning'?'#b45309':status==='pending'?'#64748b':'#0f766e';
  return <button type="button" id={`overview-kpi-${metric}`} className={`overview-kpi ${gauge?'has-gauge':''}`} aria-pressed={active} aria-controls="overview-metric-detail" disabled={disabled} onClick={()=>onOpen(metric)}><span className="overview-kpi-label">{label}</span>{gauge&&<Gauge config={gauge} status={status}/>}<strong className="overview-kpi-value">{value}</strong><span className="overview-kpi-note" style={{color:accent}}>{note}</span><span className="overview-kpi-action">{disabled?'Esperando datos':active?'Viendo desglose ↓':'Explorar por locación →'}</span></button>;
}
...
export default function OperationalOverview({allowedLocations,initialLocation='All locations',modules,onOpenModule}:Props){
  ...
  return <div style={{display:'grid',gap:16}}>
    ...
    <section aria-label="Indicadores interactivos" style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(190px,1fr))',gap:12}}>
      ...
      <Kpi {...metricProps('labor')} label="TOTAL LABOR" value={loading?'Loading…':total?money2.format(total.totalLaborCost):'Pending source'} gauge={total?{value:total.totalLaborPct,max:40,target:30,format:value=>`${value.toFixed(1)}%`}:undefined} note={total?`Hourly ${total.hourlyLaborPct.toFixed(2)}% + Salary ${total.salaryLaborPct.toFixed(2)}%`:'Hourly + salary labor combined automatically'} status={total&&total.totalLaborPct>30?'warning':total?'ready':'pending'} />
      <Kpi {...metricProps('tasks')} label="TASKS COMPLIANCE" value={tasks?(tasks.totals.total>0?`${tasks.totals.completed} / ${tasks.totals.total}`:'Sin tareas'):loading?'Loading…':tasksError?'Connection error':'Sin datos'} gauge={tasks&&tasks.totals.total>0?{value:tasks.totals.compliancePct,max:100,target:80,higherIsBetter:true,format:value=>`${value.toFixed(0)}%`}:undefined} note={tasks?'Tasks completed · Live 7shifts':tasksError} status={tasks&&tasks.totals.total>0?(tasks.totals.compliancePct>=80?'ready':'warning'):'pending'} />
      <Kpi {...metricProps('voids')} label="VOIDS" value={total?money2.format(total.voidAmount):'Pending source'} gauge={total?{value:total.voidPct,max:1,target:.5,format:value=>`${value.toFixed(2)}%`}:undefined} note="Live Toast order void calculation" status={total&&total.voidPct>.5?'warning':total?'ready':'pending'} />
      <Kpi {...metricProps('discounts')} label="DISCOUNTS" value={total?money2.format(total.discountAmount):'Pending source'} gauge={total?{value:total.discountPct,max:4,target:2,format:value=>`${value.toFixed(2)}%`}:undefined} note="Employee and Uber Eats exclusions follow bonus rules" status={total&&total.discountPct>2?'warning':total?'ready':'pending'} />
    </section>
