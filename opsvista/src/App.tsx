...
export default function App() {
  ...
  const [section,setSection] = useState<OpsVistaModule>('Action Center');
  ...
  useEffect(() => {
    if (!nav.includes(section)) setSection(nav.includes('Action Center') ? 'Action Center' : nav[0]);
  ...
  const filtered = useMemo(()=>scopedActions.filter(a=>{
  ...
  const applyVerification = (id:number,status:VerificationStatus,note:string) => {
    if (!permissions.canVerifyActions) return;
    updateAction(id,{ verificationStatus:status, verificationNote:note, verifiedAt:new Date().toISOString(), status:status==='Worked'?'Completed':status==='Did not work'?'Investigating':'Assigned' });
  };

  const escalateExternal = (item:ExternalEscalation,category:string) => {
    if (!permissions.canEscalateActions || !canAccessLocation(currentUser,item.location)) return;
    setActions(items=>{
    ...
    setSection('Action Center'); setLocation('All locations'); setSearch('');
  };

  const runRulesNow = () => {
    if (!permissions.canRunAutomation) return;
    const scopedSignals = demoAutomationSignals.filter(signal => canAccessLocation(currentUser,signal.location));
    const result = runActionRules(scopedSignals, actions);
    if (result.actions.length) setActions(items=>{ let nextId=Math.max(0,...items.map(a=>a.id)); return [...items,...result.actions.map(action=>({...action,id:++nextId,status:'Open' as const,verificationStatus:'Pending' as const}))]; });
  ...
  const isIntelligenceModule = isRamp || isLabor || isEvidence || isSettings;

  const eyebrow = isRamp?'FINANCIAL ACCOUNTABILITY':isLabor?'WORKFORCE INTELLIGENCE':isEvidence?'OPERATIONAL VERIFICATION':isSettings?'ACCESS & SECURITY':'OPERATIONAL INTELLIGENCE';
  const title = isRamp?'Gastos · Ramp Compliance':isLabor?'Horarios · Labor Intelligence':isEvidence?'Tasks · Evidence Audit':isSettings?'Configuración · Roles & Permissions':section;
  const subtitle = isRamp?'Cada gasto debe mostrar quién lo hizo, dónde pertenece, por qué se hizo y contar con la evidencia requerida.':isLabor?'Compara ventas, forecast, labor, SPLH y overtime para convertir desviaciones en acciones con impacto financiero.':isEvidence?'Verifica que las tareas realmente cumplan el estándar mediante evidencia, revisión humana, corrección y trazabilidad.':isSettings?'Controla qué puede ver y hacer cada usuario por rol y ubicación.':'Detecta qué requiere atención, entiende la causa y convierte la señal en una acción verificable.';
    ...
    <main>
      ...
      <div className="page">
        ...
        {isSettings ? <AccessControlPanel currentUser={currentUser} onChangeUser={setCurrentUser} /> : isRamp ? <RampComplianceView onEscalate={permissions.canEscalateActions?item=>escalateExternal(item,'Ramp Compliance'):undefined} /> : isLabor ? <LaborIntelligenceView allowedLocations={permissions.allLocations?undefined:allowedLocations} onEscalate={permissions.canEscalateActions?item=>escalateExternal(item,'Labor Intelligence'):undefined} /> : isEvidence ? <EvidenceAuditView allowedLocations={permissions.allLocations?undefined:allowedLocations} canReview={permissions.canReviewEvidence} reviewerName={currentUser.name} onEscalate={permissions.canEscalateActions?item=>escalateExternal(item,'Evidence Audit'):undefined} /> : <>
          ...
          <section className="score-banner"><div><span>OpsVista Access</span><strong>{currentUser.role}</strong></div><div className="score-copy"><strong>Least-privilege operational view</strong><span>This session only receives modules, locations and actions authorized for the current role.</span></div><div className="health"><span className="dot green"></span>Authorized <span className="dot amber"></span>Scoped <span className="dot red"></span>Restricted</div></section>
