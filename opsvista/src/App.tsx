...
export default function App(){
  ...
  const isDedicated=isOverview||isRamp||isLabor||isEvidence||isSettings;
  const eyebrow=isOverview?'OPERATING PERFORMANCE':isRamp?'FINANCIAL ACCOUNTABILITY':isLabor?'WORKFORCE INTELLIGENCE':isEvidence?'OPERATIONAL VERIFICATION':isSettings?'ACCESS & SECURITY':'OPERATIONAL INTELLIGENCE';
  const title=isOverview?(section==='Ventas'?'Ventas · Performance':'Resumen · Operating Performance'):isRamp?'Gastos · Ramp Compliance':isLabor?'Horarios · Labor Intelligence':isEvidence?'Tasks · Evidence Audit':isSettings?'Configuración · Roles & Permissions':section;
  const subtitle=isOverview?'Ventas, labor, task compliance, voids y descuentos en una sola vista operativa.':isRamp?'Cada gasto debe mostrar quién lo hizo, dónde pertenece, por qué se hizo y contar con la evidencia requerida.':isLabor?'Compara ventas, forecast, labor, SPLH y overtime para convertir desviaciones en acciones con impacto financiero.':isEvidence?'Verifica que las tareas realmente cumplan el estándar mediante evidencia, revisión humana, corrección y trazabilidad.':isSettings?'Controla acceso, credenciales, usuarios y permisos.':'Detecta qué requiere atención, entiende la causa y convierte la señal en una acción verificable.';
