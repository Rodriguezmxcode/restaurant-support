...
import { normalizeModule, canAccessLocation, currentAuthenticatedUser, permissionsFor, visibleLocations, type OpsVistaModule, type OpsVistaUser } from './accessControl';
import GoogleBusinessIntegrationPanel from './GoogleBusinessIntegrationPanel';
import PvControlIntegrationPanel from './PvControlIntegrationPanel';
...
import { hasLegacyWorkspace } from '../shared/tenantAccess';
...
const EvidenceAuditView=recoverableLazy(()=>import('./TasksWorkspace'));
...
const FinanceView=recoverableLazy(()=>import('./FinanceView'));
...
const TeamView=recoverableLazy(()=>import('./TeamView'));
...
const icon:Record<string,string>={Team:'♟',Resumen:'⌂',Locaciones:'▦',Ventas:'↗','Google Reviews':'✦','Local Intelligence':'⌁',Gastos:'$',Horarios:'◷',Tasks:'☑','Bono semanal':'★','Action Center':'⚡',Proyectos:'◆',Pagos:'$',Finanzas:'▥',Transferencias:'⇄',Restaurant365:'R',Integraciones:'⛓',Configuración:'⚙'};
...
const searchCatalog:SearchEntry[]=[
  ...
  {section:'Team',label:'Team',description:'Empleados, posiciones y datos de contacto',keywords:['team','employees','empleados','roster','staff','personal']},
  ...
  {section:'Action Center',label:'Centro de acciones · Action Center',description:'Acciones, prioridades, responsables e historial',keywords:['actions','acciones','alerts','alertas','incidents','responsables','seguimiento','prioridades','priorities','priority','pendientes','urgente','critical','vencidas','sin asignar','historial']},
  ...
  {section:'Finanzas',label:'Finanzas',description:'P&L mensuales, costos, resultados y fuentes por restaurante',keywords:['finanzas','finance','financial','pnl','p&l','rentabilidad','profit','monthly']},
  {section:'Pagos',label:'Pagos',description:'Solicitudes, autorizaciones y registro de pagos',keywords:['payments','payment','pago','pagos','requests','solicitudes','aprobaciones','cheques']},
...
function storedSection(){
  ...
  try{const saved=window.localStorage.getItem('opsvista-section')??window.sessionStorage.getItem('opsvista-section');return saved?normalizeModule(saved as OpsVistaModule):null;}
  catch{return null;}
}

function rememberSection(section:OpsVistaModule){
  section=normalizeModule(section);
...
export default function App(){
  ...
  return hasLegacyWorkspace(user) ? <LegacyWorkspace/> : <ClientWorkspace user={user}/>;
}
...
function LegacyWorkspace(){
  ...
  const moduleResults=useMemo<GlobalSearchResult[]>(()=>{
    ...
    return searchCatalog
      ...
      .map(entry=>{
        const operationalLocations=['Resumen','Locaciones','Ventas','Google Reviews','Local Intelligence','Gastos','Horarios','Tasks','Bono semanal','Action Center','Proyectos','Pagos','Transferencias'].includes(entry.section)?allowedLocations:[];
  ...
  const openSearchResult=(target:GlobalSearchResult)=>{
    target={...target,section:normalizeModule(target.section)};
  ...
  const eyebrow=isOverview?t('OPERATING PERFORMANCE','RENDIMIENTO OPERATIVO'):isLocations?t('MULTI-LOCATION PERFORMANCE','RENDIMIENTO MULTILOCACIÓN'):isGoogleReviews?t('REPUTATION INTELLIGENCE','INTELIGENCIA DE REPUTACIÓN'):isLocalIntelligence?t('EXTERNAL OPERATING SIGNALS','SEÑALES OPERATIVAS EXTERNAS'):isRamp?t('FINANCIAL ACCOUNTABILITY','CONTROL FINANCIERO'):isTeam?t('PEOPLE & EMPLOYMENT','EQUIPO Y EMPLEADOS'):isLabor?t('WORKFORCE INTELLIGENCE','INTELIGENCIA LABORAL'):isEvidence?t('OPERATIONAL VERIFICATION','VERIFICACIÓN OPERATIVA'):isBonus?t('PERFORMANCE INCENTIVES','INCENTIVOS DE DESEMPEÑO'):isActionCenter?t('OPERATIONAL ACCOUNTABILITY','RESPONSABILIDAD OPERATIVA'):isProjects?t('PROJECT PORTFOLIO','PORTAFOLIO DE PROYECTOS'):isFinance?t('MONTHLY FINANCIAL REPORTS','REPORTES FINANCIEROS MENSUALES'):isPayments?t('PAYMENT CONTROL','CONTROL DE PAGOS'):isTransfers?t('INVENTORY CHAIN OF CUSTODY','CADENA DE CUSTODIA DE INVENTARIO'):isRestaurant365?t('ACCOUNTING INTELLIGENCE','INTELIGENCIA CONTABLE'):isIntegrations?t('CONNECTED SYSTEMS','SISTEMAS CONECTADOS'):isSettings?t('ACCESS & SECURITY','ACCESO Y SEGURIDAD'):t('OPERATIONAL INTELLIGENCE','INTELIGENCIA OPERATIVA');
  const title=isOverview?(section==='Ventas'?t('Sales · Performance','Ventas · Performance'):t('Overview · Operating Performance','Resumen · Operating Performance')):isLocations?t('Locations · Performance Dashboard','Locaciones · Performance Dashboard'):isGoogleReviews?'Google Reviews':isLocalIntelligence?'Local Intelligence':isRamp?(currentUser.role==='Location Manager'?`${t('Ramp Expenses','Gastos Ramp')} · ${allowedLocations.join(', ')}`:t('Ramp Expenses','Gastos Ramp')):isTeam?t('Team · Employee Directory','Team · Directorio de empleados'):isLabor?t('Schedules · Labor Intelligence','Horarios · Labor Intelligence'):isEvidence?'Tasks · 7shifts & Logbook':isBonus?t('Weekly Bonus','Bono semanal'):isActionCenter?t('Action Center','Centro de acciones'):isProjects?t('Projects','Proyectos'):isFinance?t('Finance · Restaurant P&L','Finanzas · P&L por restaurante'):isPayments?t('Payments · Approval Workflow','Pagos · Approval Workflow'):isTransfers?t('Transfers · Restaurant Ledger','Transferencias · Restaurant Ledger'):isRestaurant365?'Restaurant365 · Accounting Intelligence':isIntegrations?t('Integrations · Connected Systems','Integraciones · Connected Systems'):isSettings?t('Settings · Roles & Permissions','Configuración · Roles & Permissions'):section;
  const subtitle=isOverview?t('Sales, labor, task compliance, voids and discounts in one operating view.','Ventas, labor, task compliance, voids y descuentos en una sola vista operativa.'):isLocations?t('Compare complete restaurant performance using live Toast, 7shifts and OpsVista data.','Compara el desempeño completo de cada restaurante con datos reales de Toast, 7shifts y OpsVista.'):isGoogleReviews?t('Monitor volume, rating, responses and critical reviews by restaurant from Google Business Profile.','Monitorea volumen, calificación, respuestas y reseñas críticas por restaurante desde Google Business Profile.'):isLocalIntelligence?t('Weather, traffic, incidents and nearby events translated into restaurant impact and recommendations.','Clima, tráfico, incidentes y eventos cercanos convertidos en impacto y recomendaciones por restaurante.'):isRamp?t('Track restaurant expenses, receipts and Ramp memo compliance.','Revisa los gastos de tus restaurantes y completa en Ramp los memos o recibos pendientes.'):isTeam?t('Employee roster with Toast identity, position, contact information and employment status.','Roster de empleados con identidad de Toast, posición, contacto y estatus laboral.'):isLabor?t('Compare sales, forecast, labor, SPLH and overtime to turn variances into financial actions.','Compara ventas, forecast, labor, SPLH y overtime para convertir desviaciones en acciones con impacto financiero.'):isEvidence?t('Create operational tasks and review connected 7shifts lists.','Crea tareas para tu operación y consulta las listas conectadas de 7shifts.'):isBonus?t('Weekly scorecard with Tasks, discounts, voids, overtime and eligibility rules.','Scorecard semanal con Tasks, descuentos, voids, overtime y reglas de elegibilidad.'):isActionCenter?t('Turn operating signals into assigned, verifiable and persistent actions.','Convierte señales operativas en acciones asignadas, verificables y guardadas permanentemente.'):isProjects?t('Plan initiatives with owners, dates, milestones, budget and outcomes.','Planifica iniciativas con responsables, fechas, milestones, presupuesto y resultados sin mezclarlas con alertas operativas.'):isFinance?t('Review reported sales, costs and operating results, with their sources and pending adjustments.','Consulta ventas, costos y resultados reportados, con sus fuentes y ajustes pendientes.'):isPayments?t('Managers request; Corporate approves; Administration issues approved payments only.','Managers solicitan; Corporate aprueba; Administration emite únicamente pagos aprobados, con bitácora completa.'):isTransfers?t('Track what left, what arrived, who received it, when, and any shortages between restaurants.','Registra qué salió, qué llegó, quién recibió, a qué hora y cualquier faltante o diferencia entre restaurantes.'):isRestaurant365?t('P&L, AP invoices, vendors, GL accounts and sync health, separated from daily manager work.','P&L, facturas AP, vendors, cuentas GL y salud de sincronización, separados del trabajo diario de los managers.'):isIntegrations?t('Connect and manage the external systems that feed OpsVista.','Conecta y administra las fuentes externas que alimentan OpsVista.'):isSettings?t('Manage access, users and permissions.','Controla acceso, usuarios y permisos.'):t('This module source is unavailable.','La fuente de este módulo no está disponible.');
  ...
  return <div className={`app-shell ${previewUser?'user-preview-active':''} ${sidebarCollapsed?'sidebar-collapsed':''}`}>
    <aside className="sidebar">
      ...
      <nav aria-label="Módulos de OpsVista">{nav.map(item=>{const labels:Record<string,[string,string]>={Resumen:['Overview','Resumen'],Locaciones:['Locations','Locaciones'],Ventas:['Sales','Ventas'],'Google Reviews':['Google Reviews','Google Reviews'],'Local Intelligence':['Local Intelligence','Local Intelligence'],Gastos:['Ramp Expenses','Gastos Ramp'],Team:['Team','Equipo'],Horarios:['Schedules','Horarios'],Tasks:['Tasks','Tasks'],'Bono semanal':['Weekly Bonus','Bono semanal'],'Action Center':['Action Center','Centro de acciones'],Proyectos:['Projects','Proyectos'],Finanzas:['Finance','Finanzas'],Pagos:['Payments','Pagos'],Transferencias:['Transfers','Transferencias'],Restaurant365:['Restaurant365','Restaurant365'],Integraciones:['Integrations','Integraciones'],Configuración:['Settings','Configuración']};const pair=labels[item]||[item,item];const label=t(pair[0],pair[1]);return <button key={item} className={section===item?'active':''} onClick={()=>{rememberSection(item);setSearchTarget(null);setSection(item);}} aria-label={sidebarCollapsed?label:undefined} title={sidebarCollapsed?label:undefined}><span className="nav-icon">{icon[item]}</span><span className="nav-label">{label}</span></button>})}</nav>
