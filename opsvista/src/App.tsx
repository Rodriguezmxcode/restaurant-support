...
const initialActions: ActionItem[] = [
  { id:1, location:'Orange', category:'Labor', title:'Labor projected above target', severity:'High', status:'Open', signal:'Projected labor is 24.8% while the current target is 21.0%.', cause:'Sales are trending 13% below forecast while closing coverage remains above comparable demand.', recommendation:'Review closing coverage and reduce approximately 9 labor hours if demand does not recover.', impact:'$176 estimated savings tonight' },
  { id:2, location:'Fairfield', category:'Expenses', title:'Ramp transactions missing evidence', severity:'Medium', status:'Open', signal:'3 transactions are missing receipts or required memo detail.', cause:'Transactions were imported but compliance fields are incomplete.', recommendation:'Assign the cardholders to upload receipts and complete the memo before the compliance deadline.', impact:'$486.73 requiring evidence' },
  { id:3, location:'Stamford', category:'Tasks', title:'Closing evidence needs review', severity:'Medium', status:'Open', signal:'2 closing checklist items contain rejected or incomplete evidence.', cause:'Submitted evidence does not clearly verify the required standard.', recommendation:'Request resubmission and verify the corrected evidence before closing the shift workflow.', impact:'2 unresolved evidence items' },
  { id:4, location:'Danbury', category:'Overtime', title:'Overtime exposure increasing', severity:'High', status:'Assigned', signal:'Current scheduling pattern can push the kitchen above the weekly overtime threshold.', cause:'A small group of cooks is absorbing both peak coverage and late closing shifts.', recommendation:'Move closing coverage to employees with lower accumulated hours and audit punches before payroll close.', impact:'$428 projected overtime exposure', owner:'Manager on duty' },
];
    ...
    <main>
      ...
      <div className="page">
        ...
        {isRamp ? <RampComplianceView onEscalate={item=>escalateExternal(item,'Ramp Compliance')} /> : isLabor ? <LaborIntelligenceView onEscalate={item=>escalateExternal(item,'Labor Intelligence')} /> : isEvidence ? <EvidenceAuditView onEscalate={item=>escalateExternal(item,'Evidence Audit')} /> : <>
          ...
          <section className="score-banner"><div><span>OpsVista Score</span><strong>87</strong><small>/100</small></div><div className="score-copy"><strong>6 locations healthy · 1 requires attention</strong><span>Score combines labor, tasks, exceptions, compliance and unresolved operational actions.</span></div><div className="health"><span className="dot green"></span>Healthy <span className="dot amber"></span>Watch <span className="dot red"></span>Action</div></section>
          <div className="content-grid">
            <section className="panel action-list-panel"><div className="panel-header"><div><h2>Needs Action</h2><p>Prioritized by operational and financial impact.</p></div><span className="count-pill">{filtered.length}</span></div><div className="action-list">{filtered.map(a=><button key={a.id} className={`action-row ${selectedId===a.id?'selected':''}`} onClick={()=>setSelectedId(a.id)}><div className={`severity ${a.severity.toLowerCase()}`}>{a.severity==='High'?'!':a.severity==='Medium'?'•':'○'}</div><div className="action-main"><div className="action-meta"><span>{a.location}</span><span>•</span><span>{a.category}</span></div><strong>{a.title}</strong><p>{a.signal}</p><div className="action-footer"><span className={`status ${a.status.toLowerCase()}`}>{a.status}</span><span className="impact">{a.impact}</span></div></div><span className="chev">›</span></button>)}</div></section>
            {selected&&<section className="panel detail-panel"><div className="detail-top"><div><span className={`severity-label ${selected.severity.toLowerCase()}`}>{selected.severity} priority</span><h2>{selected.title}</h2><p>{selected.location} · {selected.category}</p></div><button className="icon-btn">•••</button></div><div className="detail-block"><label>SIGNAL</label><p>{selected.signal}</p></div><div className="detail-block"><label>LIKELY CAUSE</label><p>{selected.cause}</p></div><div className="detail-block recommendation"><label>OPSVISTA RECOMMENDATION</label><p>{selected.recommendation}</p></div><div className="impact-box"><span>Estimated impact</span><strong>{selected.impact}</strong></div>{selected.owner&&<div className="owner-box"><div className="avatar small">{selected.owner.split(' ').map(x=>x[0]).join('').slice(0,2)}</div><div><span>Owner</span><strong>{selected.owner}</strong></div></div>}<div className="action-buttons"><button className="primary" onClick={()=>updateAction(selected.id,{status:'Assigned',owner:selected.owner??'Location Manager'})}>Assign owner</button><button onClick={()=>updateAction(selected.id,{status:'Assigned',owner:selected.owner??'Location Manager'})}>Create task</button><button onClick={()=>updateAction(selected.id,{status:'Investigating'})}>Investigate</button><button onClick={()=>updateAction(selected.id,{status:'Dismissed'})}>Dismiss</button></div><div className="verification"><strong>Verification loop</strong><p>Once the action is completed, OpsVista should compare the next measured result against the original signal and record whether the intervention worked.</p><button onClick={()=>updateAction(selected.id,{status:'Completed'})}>Mark completed for demo</button></div></section>}
          </div>
          <section className="panel roadmap-panel"><div className="panel-header"><div><h2>Next intelligence layers</h2><p>These modules plug into the same Action Center workflow.</p></div></div><div className="roadmap-grid"><div><span>01</span><strong>Evidence Audit</strong><p>Photo proof, approve/reject, resubmission, before/after and audit history.</p></div><div><span>02</span><strong>Ramp Compliance</strong><p>Cardholder, department, memo, receipt, deadline, duplicate and policy signals.</p></div><div><span>03</span><strong>Labor Intelligence</strong><p>Forecast vs schedule, overtime exposure, suggested cuts and verified savings.</p></div><div><span>04</span><strong>OpsVista AI Copilot</strong><p>Explain why a KPI moved and turn recommendations into assignable actions.</p></div></div></section>
        </>}
      </div>
    </main>
  </div>;
