...
export default function App(){
  ...
  const actionVizRows=useMemo(()=>allowedLocations.map(location=>{const locationActions=scopedActions.filter(action=>action.location===location&&!['Completed','Dismissed'].includes(action.status));const high=locationActions.filter(action=>action.severity==='High').length;const averagePriority=locationActions.length?locationActions.reduce((sum,action)=>sum+(action.priorityScore??0),0)/locationActions.length:0;const status:'good'|'watch'|'bad'=high?'bad':locationActions.length?'watch':'good';return{location,primary:locationActions.length,secondary:averagePriority,status};}),[allowedLocations,scopedActions]);
    ...
    <main>
      ...
      <div className="page">
        ...
        {isOverview?<OperationalOverview allowedLocations={allowedLocations} allLocations={permissions.allLocations}/>:isLocations?<LocationDashboard allowedLocations={allowedLocations} allLocations={permissions.allLocations} onOpenTasks={()=>setSection('Tasks')} onOpenLabor={()=>setSection('Horarios')}/>:isLocalIntelligence?<LocalIntelligenceView allowedLocations={allowedLocations}/>:isActionCenter?<ActionCenterView currentUser={currentUser} allowedLocations={allowedLocations}/>:isProjects?<ProjectsView currentUser={currentUser} allowedLocations={allowedLocations} canSeeFinancialImpact={permissions.canSeeFinancialImpact}/>:isPayments?<PaymentRequestsView currentUser={currentUser} allowedLocations={allowedLocations}/>:isTransfers?<TransferLedgerView/>:isSettings?<><ChangePasswordPanel/><InvitationManager currentUser={currentUser}/><AccessControlPanel currentUser={currentUser} onChangeUser={setCurrentUser}/></>:isRamp?<RampComplianceView onEscalate={permissions.canEscalateActions?item=>escalateExternal(item,'Ramp Compliance'):undefined}/>:isLabor?<LaborIntelligenceView allowedLocations={allowedLocations} onEscalate={permissions.canEscalateActions?item=>escalateExternal(item,'Labor Intelligence'):undefined}/>:isBonus?<WeeklyBonusLivePanel allowedLocations={permissions.allLocations?undefined:allowedLocations} canImportReviews={currentUser.role==='Founder'||currentUser.role==='Corporate'}/>:isEvidence?<EvidenceAuditView allowedLocations={permissions.allLocations?undefined:allowedLocations} canReview={permissions.canReviewEvidence} reviewerName={currentUser.name} onEscalate={permissions.canEscalateActions?item=>escalateExternal(item,'Evidence Audit'):undefined}/>:<>
          ...
          <MaxDataInsights title="Acciones por locación" subtitle="Carga operativa y prioridad media del Action Center; cada visualización funciona como filtro cruzado." rows={actionVizRows} primaryLabel="Acciones abiertas" secondaryLabel="Prioridad media" primaryFormat={value=>value.toFixed(0)} secondaryFormat={value=>`${value.toFixed(0)} / 100`}/>
