...
export default function App(){
  ...
  const escalateExternal=async(item:ExternalEscalation,category:string)=>{if(!permissions.canEscalateActions||!canAccessLocation(currentUser,item.location))throw new Error('Action is outside your authorized locations');const automationKey=item.automationKey||`external::${category}::${item.location}::${item.title}::${item.signal}`;const response=await fetch('/api/workflows?resource=actions',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({...item,category,automationKey,automated:true,priorityScore:item.priorityScore??(item.severity==='High'?80:item.severity==='Medium'?50:25),sources:item.sources??[category],detectedAt:item.detectedAt??new Date().toISOString()})});const body=await response.json().catch(()=>({})) as {action?:{id:string};error?:string};if(!response.ok||!body.action)throw new Error(body.error||'Action could not be saved');setSearchTarget({id:`action-${body.action.id}`,section:'Action Center',label:item.title,description:`${item.location} · ${item.ownerName||'Unassigned'}`,badge:'ACTION',recordId:body.action.id,location:item.location});setSection('Action Center');setSearch('');return body.action;};
