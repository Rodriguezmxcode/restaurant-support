...
export default function App(){
  ...
  return <div className={`app-shell ${previewUser?'user-preview-active':''}`}>
    <aside className="sidebar">
      ...
      <nav>{nav.map(item=><button key={item} className={section===item?'active':''} onClick={()=>{rememberSection(item);setSection(item);}}><span className="nav-icon">{icon[item]}</span>{item==='Gastos'?'Gastos Ramp':item}</button>)}</nav>
