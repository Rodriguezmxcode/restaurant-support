...
function restore(){const p=page();if(!p)return;for(const child of [...p.children] as HTMLElement[]){if(child===host)continue;if(child.dataset.priceWatchHidden==='1'){child.style.removeProperty('display');delete child.dataset.priceWatchHidden;}}if(host){host.style.display='none';host.hidden=true;}active=false;}
function show(){const p=page(),tab=tabs();if(!p||!tab||!host)return;active=true;for(const button of [...tab.querySelectorAll<HTMLButtonElement>('button')]){const selected=button.dataset.priceWatchTab==='1';button.setAttribute('aria-selected',selected?'true':'false');button.classList.toggle('active',selected);}for(const child of [...p.children] as HTMLElement[]){if(child===tab||child===host||child.getAttribute('aria-label')==='Guardado y sincronización')continue;child.dataset.priceWatchHidden='1';child.style.display='none';}host.hidden=false;host.style.display='block';host.scrollIntoView({block:'start',behavior:'smooth'});}
...
function handleClick(event:MouseEvent){const target=event.target instanceof Element?event.target.closest<HTMLButtonElement>('button'):null;if(!target)return;const tab=tabs();if(!tab||!tab.contains(target))return;if(target.dataset.priceWatchTab==='1'||target.textContent?.trim()==='Price Watch'){event.preventDefault();event.stopImmediatePropagation();sync();show();return;}if(active)restore();}
