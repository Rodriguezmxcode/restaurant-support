...
try {
  ...
  for (const name of ['shared/nativeTasks', 'server/nativeTaskService', 'server/nativeTaskService.test', 'src/actionAssignments', 'server/actionAssignments.test']) {
  ...
  const result = spawnSync(process.execPath, ['--test', join(output, 'server/nativeTaskService.test.js'), join(output, 'server/actionAssignments.test.js')], { stdio: 'inherit' });
