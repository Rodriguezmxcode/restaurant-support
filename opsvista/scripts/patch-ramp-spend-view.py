...
path = Path('opsvista/src/RampComplianceView.tsx')
text = path.read_text()

old = "type RampPeriod = 'today' | 'yesterday' | 'this_week' | 'prior_week' | 'last_30' | 'custom';"
new = old + "\ntype RampSpendView = 'total' | 'exposed';"
if old not in text:
    raise SystemExit('RampPeriod anchor not found')
text = text.replace(old, new, 1)

old = "  const [assignmentError, setAssignmentError] = useState('');"
new = old + "\n  const [spendView, setSpendView] = useState<RampSpendView>('exposed');"
if old not in text:
    raise SystemExit('state anchor not found')
text = text.replace(old, new, 1)

old = "  const locationRows = useMemo(() => groupRampCompliance(results, 'department'), [results]);\n  const overdueRows = useMemo(() => results.filter(tx => tx.overdue), [results]);"
new = """  const locationRows = useMemo(() => groupRampCompliance(results, 'department'), [results]);
...
if old not in text:
    raise SystemExit('MaxDataInsights block not found')
text = text.replace(old, new, 1)
path.write_text(text)

css_path = Path('opsvista/src/rampCompliance.css')
css = css_path.read_text()
marker = '/* Ramp spend audit reconciliation */'
if marker not in css:
    css += '''

/* Ramp spend audit reconciliation */
.ramp-spend-audit{border:1px solid #c8d7e8;border-radius:18px;background:#fff;overflow:hidden}.ramp-spend-audit-head{display:flex;align-items:center;justify-content:space-between;gap:18px;padding:18px 22px;background:linear-gradient(90deg,#f8fbff,#eef5ff);border-bottom:1px solid #d8e4f0}.ramp-spend-audit-head>div:first-child{display:grid;gap:4px}.ramp-spend-audit-head span{font-size:11px;font-weight:800;letter-spacing:.14em;color:#2a68c7}.ramp-spend-audit-head strong{font-size:18px;color:#10243f}.ramp-spend-audit-head small{color:#61738d}.ramp-spend-toggle{display:flex;gap:6px;padding:4px;border:1px solid #c7d7e8;border-radius:999px;background:#fff}.ramp-spend-toggle button{border:0;background:transparent;border-radius:999px;padding:8px 13px;font-weight:750;color:#40546f;cursor:pointer}.ramp-spend-toggle button.active{background:#1d65d8;color:#fff}.ramp-spend-reconciliation{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:0}.ramp-spend-reconciliation article{padding:16px 20px;border-right:1px solid #e1e9f2;display:grid;gap:4px}.ramp-spend-reconciliation article:last-child{border-right:0}.ramp-spend-reconciliation span{font-size:11px;font-weight:800;letter-spacing:.1em;color:#60738d}.ramp-spend-reconciliation strong{font-size:23px;color:#10243f}.ramp-spend-reconciliation small{color:#6c7e93}.ramp-spend-reconciliation .needs-classification{background:#fff7ed}.ramp-spend-reconciliation .needs-classification strong{color:#c2410c}.ramp-spend-reconciliation .out-of-balance{background:#fff1f2}.ramp-spend-reconciliation .out-of-balance strong{color:#b42318}.ramp-spend-reconciliation .balanced{background:#eefbf4}.ramp-spend-reconciliation .balanced strong{color:#16794a}@media(max-width:850px){.ramp-spend-audit-head{align-items:flex-start;flex-direction:column}.ramp-spend-reconciliation{grid-template-columns:repeat(2,minmax(0,1fr))}.ramp-spend-reconciliation article:nth-child(2){border-right:0}.ramp-spend-reconciliation article:nth-child(-n+2){border-bottom:1px solid #e1e9f2}}@media(max-width:520px){.ramp-spend-reconciliation{grid-template-columns:1fr}.ramp-spend-reconciliation article{border-right:0;border-bottom:1px solid #e1e9f2}.ramp-spend-reconciliation article:last-child{border-bottom:0}.ramp-spend-toggle{width:100%}.ramp-spend-toggle button{flex:1}}
'''
